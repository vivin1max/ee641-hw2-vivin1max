...
from training_utils import train_hierarchical_vae, sample_diverse_patterns, analyze_posterior_collapse

def kl_annealing_schedule(epoch, method='cyclical'):
    # KL annealing
    if method == 'linear':
        return min(epoch / 50.0, 1.0)
    elif method == 'cyclical':
        cycle_len = 20
        cycle_pos = epoch % cycle_len
        return cycle_pos / cycle_len
    elif method == 'sigmoid':
        return 1.0 / (1.0 + np.exp(-0.1 * (epoch - 50)))
    else:
        return 1.0

def temperature_annealing_schedule(epoch):
    # Temperature annealing
    return max(0.5, 1.0 - epoch / 200.0)
...
def main():
    ...
    config = {
        ...
        'data_dir': r'C:\Users\Vivin\ee641-hw2-vivin1max\data\drums',
